...
const BattleComponent: React.FC<BattleComponentProps> = ({
  ...
  const renderLocationSelection = () => (
    <div style={{ textAlign: "center" }}>
      ...
      <h3 style={{ color: "var(--secondary-gold)", marginBottom: "0.75rem" }}>Ready for Battle</h3>
  ...
  const renderElementSelection = () => (
    <div style={{ textAlign: "center" }}>
      ...
      <button className="btn-primary" onClick={onStartMatchmaking} disabled={!player.selectedElement}>
    ...
    <div style={{ textAlign: "center" }}>
      <div className="battle-display">
        <div className="battle-choice">
          <div className="battle-emoji">{player.selectedElement ? ELEMENTS[player.selectedElement].emoji : "❓"}</div>
          <div style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>You</div>
        </div>
        <div className="battle-vs">VS</div>
        <div className="battle-choice">
          <div className="battle-emoji">{opponentElement ? ELEMENTS[opponentElement].emoji : "❓"}</div>
    ...
    <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              ...
              <div>
                ...
                <div style={{ fontWeight: 600, color: "var(--secondary-gold)" }}>{player.mana}</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              <span style={{ fontSize: "1.2rem" }}>🏆</span>
              <div>
                ...
                <div style={{ fontWeight: 600, color: "var(--success)" }}>{player.wins}</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              <span style={{ fontSize: "1.2rem" }}>🔥</span>
              <div>
                ...
                <div style={{ fontWeight: 600, color: "var(--text-primary)" }}>{player.winStreak}</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              <span style={{ fontSize: "1.2rem" }}>💀</span>
              <div>
                ...
                <div style={{ fontWeight: 600, color: "var(--error)" }}>{player.losses}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            {!["menu", "elementSelection", "battle", "result", "matchmaking"].includes(gamePhase) && (
              <div style={{ textAlign: "center", color: "var(--error)" }}>Unknown game phase: {gamePhase}</div>
            )}
          </div>
        </div>
      </main>
      ...
      >
        ...
        <div style={{ textAlign: "center", color: "var(--text-secondary)" }}>
          ...
          <div style={{ fontSize: "0.85rem", fontStyle: "italic" }}>Finding a worthy challenger</div>
        </div>
      </Modal>
                  ...
                  {getAvailableElementals(player.selectedElement).map((rarity) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="elemental-protection">{Math.round(elemental.protection * 100)}% protection</div>
                      </button>
                    );
                  })}
                </div>
      ...
      <Modal isOpen={gamePhase === "battle"} onClose={() => {}} title="⚔️ Battle">
        <div className="battle-display">
          <div className="battle-choice">
            <div className="battle-emoji">{player.selectedElement ? ELEMENTS[player.selectedElement].emoji : "❓"}</div>
            <div
