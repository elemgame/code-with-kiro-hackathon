...
const StepByStepTutorial: React.FC<StepByStepTutorialProps> = ({
  ...
  const [highlightPosition, setHighlightPosition] = useState({ top: 0, left: 0, width: 0, height: 0, borderRadius: '8px' });
  ...
  const updateAllPositions = useCallback((element: HTMLElement) => {
    ...
    const borderRadius = computedStyle.borderRadius || computedStyle.borderTopLeftRadius || '8px';
  ...
  useEffect(() => {
    ...
    const findTarget = () => {
      ...
      if (element) {
        // Add debug logging
        console.log('Found target element:', element, 'for selector:', currentStep.targetSelector);
        ...
        console.log('Added tutorial-target class, element classes:', element.className);
        console.log('Body has tutorial-active:', document.body.classList.contains('tutorial-active'));
        ...
        if (currentStep.triggerNext === 'auto') {
          console.log('Auto-proceeding to next step for:', currentStep.id);
          // Use a separate useEffect for auto-proceeding to avoid dependency issues
        }
      } else {
        console.log('No element found for selector:', currentStep.targetSelector);
      }
    };
  ...
  }, [isActive, currentStep, updateAllPositions]);
  ...
  const nextStep = useCallback(() => {
    ...
    if (currentOverallIndex < steps.length - 1) {
      ...
      // eslint-disable-next-line no-console
      console.log('Moving to next step:', nextStepData?.id, 'phase:', nextStepData?.phase);

      if (nextStepData) {
        // If we're moving to a different phase, reset step index to 0
        if (nextStepData.phase !== currentPhase) {
          // eslint-disable-next-line no-console
          console.log('Phase change detected, resetting step index to 0');
          setCurrentStepIndex(0);
        } else {
          // Same phase, increment step index
          // eslint-disable-next-line no-console
          console.log('Same phase, incrementing step index');
  ...
  useEffect(() => {
    ...
    const handleTargetClick = (event: Event) => {
      console.log('Tutorial click detected:', {
        stepId: currentStep.id,
        triggerNext: currentStep.triggerNext,
        currentPhase,
           ...
           <div className="tutorial-step-progress">
             ...
             <div className="tutorial-step-progress-bar">
               <div
                 ...
                 style={{ width: `${((steps.findIndex(s => s.id === currentStep.id) + 1) / steps.length) * 100}%` }}
               />
             </div>
           </div>
...
export const BATTLE_TUTORIAL_STEPS: TutorialStep[] = [
  // Step 1: Location selection
  {
    ...
    description: 'Click on "Free Practice" to start your first battle. This location costs no mana and is perfect for learning!',
    phase: 'menu',
    position: 'bottom',
    actionRequired: true,
    triggerNext: 'click',
  ...
  // Step 2: Element selection
  {
    ...
    description: 'Fire element is recommended for beginners. Click on Fire to select it! Fire is strong against Earth but weak against Water.',
  ...
  // Step 3: Elemental selection
  {
    ...
    description: 'Click on your Common elemental to select it for battle. Common elementals have unlimited uses - perfect for practice!',
