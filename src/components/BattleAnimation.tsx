...
const BattleAnimation: React.FC<BattleAnimationProps> = ({ gameState, opponentElement, onAnimationComplete }) => {
  const [phase, setPhase] = useState<"intro" | "elements" | "elementals" | "clash" | "result">("intro");
  ...
  const playerElement = player.selectedElement!;
  const playerElemental = player.selectedElemental ? getElementalData(playerElement, player.selectedElemental) : null;
            ...
            <div className="opponent-profiles">
              ...
              <div className="opponent-profile slide-in-right">
                ...
                <div className="profile-info">
                  ...
                  <div className="profile-level">Level {currentOpponent?.level}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {phase === "elements" && (
          <div className="elements-battle">
            ...
            <div className="elements-arena">
              <div className={`element-fighter player ${showPlayerElement ? "enter-battle" : ""}`}>
                <div
                ...
                >
                  <span className="element-emoji">{ELEMENTS[playerElement].emoji}</span>
                </div>
                <div className="element-name">{ELEMENTS[playerElement].name}</div>
              </div>

              <div className="battle-lightning">⚡</div>

              <div className={`element-fighter opponent ${showOpponentElement ? "enter-battle" : ""}`}>
                <div
                  className="element-orb"
                  style={{
                ...
                >
                  <span className="element-emoji">{ELEMENTS[opponentElement].emoji}</span>
                </div>
                <div className="element-name">{ELEMENTS[opponentElement].name}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {phase === "elementals" && (
          <div className="elementals-battle">
            ...
            <div className="elementals-arena">
              <div className={`elemental-fighter player ${showPlayerElemental ? "summon-elemental" : ""}`}>
                {playerElemental ? (
                  <>
                    <div className="elemental-aura player-aura">
                      <span className="elemental-emoji">{playerElemental.emoji}</span>
                    </div>
                    <div className="elemental-info">
                      <div className="elemental-name">{playerElemental.name}</div>
                      <div className="protection-bar">
                        <div
                          className="protection-fill"
                          style={{ width: `${playerElemental.protection * 100}%` }}
                        ></div>
                        <span className="protection-text">{Math.round(playerElemental.protection * 100)}%</span>
                      </div>
                    </div>
                  </>
                ) : (
              ...
              <div className={`elemental-fighter opponent ${showOpponentElemental ? "summon-elemental" : ""}`}>
                {opponentElemental ? (
                  <>
                    <div className="elemental-aura opponent-aura">
                      <span className="elemental-emoji">{opponentElemental.emoji}</span>
                    </div>
                    <div className="elemental-info">
                      <div className="elemental-name">{opponentElemental.name}</div>
                      <div className="protection-bar">
                        <div
                          className="protection-fill opponent-fill"
                          style={{ width: `${opponentElemental.protection * 100}%` }}
                        ></div>
                        <span className="protection-text">{Math.round(opponentElemental.protection * 100)}%</span>
                      </div>
                    </div>
                  </>
                ) : (
